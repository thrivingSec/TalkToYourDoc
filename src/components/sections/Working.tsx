...
const Working = () => {
  ...
  return (
    <section className="relative py-32 overflow-hidden">
      ...
      <div className="mx-auto max-w-7xl px-6 flex flex-col gap-24" id='working'>
        ...
        <div className="text-center max-w-3xl mx-auto">
          ...
          <p className="text-muted-foreground mt-6 text-lg tracking-tight">
            {data.description}
          </p>
        </div>
        ...
        <div className="relative flex flex-col gap-16">
          ...
          {data.sections.map((section, i) => (
            <div key={i} className="relative flex gap-8 items-start">
              ...
              {/* node */}
              <div className="relative z-10 flex items-center justify-center h-12 w-12 rounded-full bg-linear-to-br from-primary via-accent to-secondary text-white font-semibold shadow-lg">
                {i + 1}
              </div>
              ...
              <div className="flex-1 rounded-2xl border border-border bg-card/60 backdrop-blur-sm p-8 shadow-xl hover:shadow-2xl dark:hover:shadow-accent transition">
                ...
                <ul className="flex flex-col gap-3">
                  {section.steps.map((step, j) => (
                    <li
                      key={j}
                      className="text-muted-foreground text-sm lg:text-md flex gap-3"
                    >
